...
| 类型 | 说明 | 示例 |
|------|------|------|
| `BIGINT` | 主键、大整数 | `id BIGINT` |
| `INT` | 普通整数 | `status INT` |
| `DATETIME` | 时间戳（无时区） | `create_time DATETIME` |
| `TIMESTAMP` | 时间戳（有时区） | `update_time TIMESTAMP` |
| `VARCHAR(255)` | 字符串（长度根据实际） | `name VARCHAR(50)` |
| `DECIMAL(10,2)` | 金额、精确小数 | `amount DECIMAL(10,2)` |
| `TINYINT` | 布尔值/状态 | `is_deleted TINYINT` |
