...
@Service("kingbase")
public class KingbaseCollector extends JdbcManager implements IBaseCollector {

    private final String databaseType = "54321";
    ...
    public IPage<TableEntity> selectDbTableList(IPage<TableEntity> page, DatabaseEntity database, TableQueryForm params) {
        ...
        String sql = String.format(sqlItem.getSql(),database.getSchemaName(),database.getDatabaseName()) ;
    ...
    public List<GenTableColumnEntity> selectDbTableColumnsByName(DatabaseEntity database, String tableName) {
        ...
        List<String> columns = sqlItem.getColumns();
        List<Map<String, Object>> maps = this.queryMultipleRows(database,String.format(sqlItem.getSql(),database.getSchemaName(),database.getDatabaseName()) , Collections.singletonList(tableName));
